...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs) {
  ...
  */
  vector<double> solution_return;

  for (int ii = 0; ii < N; ii++) {
    solution_return.push_back(solution.x[x_start + ii]);
  }
  for (int ii = 0; ii < N; ii++) {
    solution_return.push_back(solution.x[y_start + ii]);
  }

  solution_return.push_back(solution.x[delta_start]);
  solution_return.push_back(solution.x[a_start]);
  /*
